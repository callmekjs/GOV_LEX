...
> 공개 API의 본문 제공 일관성 문제로 MVP에선 메타데이터 ID 해시 방식을 채택했고, Phase 2에서 source_type별 본문 해시로 승격 예정입니다 ([pipline_upgrade.md Phase 2-2](pipline_upgrade.md), [schema_v1.md §6](docs/schema_v1.md#6-content_hash-생성-규칙)).
...
(현재 **19개** 테스트 — 스키마·해시·QA 룰·누적 실패 로그 등.)
...
- `tests/test_qa_rules.py` (`pytest tests/ -v` → **19개**)
...
| 테스트 | 0개 | **19개 전부 PASS** | `pytest tests/ -v` |
...
| 내부 데이터 표준에 맞춘 구조화·정제 | 9필드 canonical schema + `source_type` 5개 분류 체계 + pydantic 자동 검증 |
| 백엔드 개발자와 협업한 파이프라인 설계 | 모듈 분리(ingest / core / qa) + 소스별 어댑터 |
| 가공된 데이터 품질 검증(QA)·정확도/일관성 | `qa/rules.py` R01·R02·R05 + 자동 품질 리포트 |
| 데이터 관리 프로세스 개선·자동화 | run_id 실행 기록 + `seen_store` 영구 중복 방지 + pytest 19개 |
...
### Week 1–2 (완료 기준)
- run_id 구조, 9필드 스키마, KR 법령 API + US Congress + 국회 API 연결
- `docs.jsonl` 통합 저장, QA 룰·품질 리포트, `seen_hashes` 영구 기억
- 입법 프로세스 ↔ `source_type` 매핑 정의
- README·소스 카탈로그·테스트(19개)

### Week 3+
- 국회 회의록 API 연결 (`source_type: minutes` 수집)
- 국가법령정보 `decree`, `rule` 확장
