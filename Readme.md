...
- **초점 문서**: 법·입법 공개 데이터
- **파이프라인에 연결된 소스 (2개)**:
  - 한국 국가법령정보 Open API (`kr_law.py`)
  - 미국 Congress.gov API v3 (`us_congress.py`)
...
- **2개국·2개 소스**만으로도 스키마 통일·QA·실행 추적을 끝까지 보여줄 수 있음
- API 기반이라 재현·문서화가 쉬움
...
# 프로젝트 루트에 .env 생성 후 LAW_GO_KR_OC, CONGRESS_GOV_API_KEY 입력
```
...
| 수집 소스 | 1개국 1개 소스 | **2개국 2개 소스** | `docs/source_catalog.md` |
| 표준화 커버리지 | 단일 포맷 | **61건 전부 동일 9필드** | `data_index/normalized/docs.jsonl` |
...
|---|---|
| 글로벌 법률·규제 공개 데이터 리서치·확보 | `docs/source_catalog.md` — 2개국 2개 소스 + 라이선스·제한 명시 |
...
### Week 1–2 (완료 기준)
- run_id 구조, 9필드 스키마, KR 법령 API + US Congress 연결
